...
export async function depositFunds(id,amount,description='Пополнение администратором'){ const n=Math.abs(Number(amount)); if(!n)throw Error('Введите корректную сумму'); return rpc('admin_balance_operation',{p_amount:n,p_description:description,p_player_id:id}); }
export async function withdrawFunds(id,amount,description='Снятие администратором'){ const n=Math.abs(Number(amount)); if(!n)throw Error('Введите корректную сумму'); return rpc('admin_balance_operation',{p_amount:-n,p_description:description,p_player_id:id}); }
export async function issueFine(id,amount,reason){ const n=Number(amount); if(!n||n<=0)throw Error('Введите сумму штрафа'); return rpc('admin_issue_penalty',{p_amount:n,p_player_id:id,p_reason:reason||'Штраф',p_title:reason||'Штраф'}); }
export async function issueCredit(id,amount,interest=0){ const n=Number(amount); if(!n||n<=0)throw Error('Введите сумму кредита'); return rpc('issue_credit',{p_user_id:id,p_amount:n,p_interest:Number(interest)||0}); }
...
export async function sendNotification(id,title,message,kind='info'){ if(!message?.trim())throw Error('Введите текст уведомления'); return rpc('bank_create_notification',{p_user_id:id,p_title:title||'Сообщение банка',p_message:message,p_kind:kind}); }
export async function markNotificationRead(id){ return rpc('mark_bank_notification_read',{p_notification_id:id}); }
